...
namespace SD.Toolkits.EntityFramework.Extensions
{
    ...
    public sealed class QueryBuilder<T> where T : class
    {
        ...
        {
            // zip parameters (expressionMap from parameters of right to parameters of left)
            IDictionary<ParameterExpression, ParameterExpression> expressionMap = left.Parameters
            ...
            ParameterExpressionVisitor parameterExpressionVisitor = new ParameterExpressionVisitor(expressionMap);
    ...
    internal class ParameterExpressionVisitor : ExpressionVisitor
    {
        ...
        /// 参数表达式映射
        /// </summary>
        private readonly IDictionary<ParameterExpression, ParameterExpression> _expressionMap;

        /// <summary>
        /// 构造器
        /// </summary>
        /// <param name="expressionMap">参数表达式映射</param>
        public ParameterExpressionVisitor(IDictionary<ParameterExpression, ParameterExpression> expressionMap)
        {
            this._expressionMap = expressionMap;
        }
        ...
        {
            if (this._expressionMap.TryGetValue(expression, out ParameterExpression replacement))
            {
