...
namespace SD.Toolkits.Sql.Firebird
{
    /// <summary>
    /// Firebird数据库访问助手
    /// </summary>
    public sealed class FbSqlHelper : ISqlHelper
    {
        #region # 字段及构造器
        ...
        public FbSqlHelper(string connectionString)
        {
            #region # 验证
        ...
        #region # 创建连接方法 —— FbConnection CreateConnection()
        /// <summary>
        /// 创建连接方法
        /// </summary>
        ...
        {
            return new FbConnection(this._connectionString);
        }
        #endregion
        ...
        private int ExecuteNonQuery(string sql, CommandType type, params IDbDataParameter[] args)
        {
            ...
            int rowCount;
            using (FbConnection conn = this.CreateConnection())
            {
                FbCommand cmd = new FbCommand(sql, conn) { CommandType = type };
        ...
        private DataTable GetDataTable(string sql, CommandType type, params IDbDataParameter[] args)
        {
            ...
            using (FbConnection conn = this.CreateConnection())
            {
                FbDataAdapter adapter = new FbDataAdapter(sql, conn) { SelectCommand = { CommandType = type } };
        ...
        private DataSet GetDataSet(string sql, CommandType type, params IDbDataParameter[] args)
        {
            ...
            using (FbConnection conn = this.CreateConnection())
            {
                ...
                using (FbDataAdapter adapter = new FbDataAdapter(sql, conn))
                {
                    adapter.SelectCommand.Parameters.AddRange(args);
